...
export function MixerEditModal({
	mixer,
	open,
	onOpenChange,
	onDelete,
	...
	useEffect(() => {
		if (open && mixer) {
			setEditedName(mixer.name);
			setIsOpened(mixer.opened);
			setPurchaseDate(mixer.purchase_date || null);
			setOpenDate(mixer.open_date || null);
			setPrice(mixer.price?.toFixed(2) || "");
			setHasChanges(false);
		}
	}, [open, mixer]);

	const handleSave = async () => {
		if (!mixer || !editedName.trim()) return;

		try {
			setIsSaving(true);
			await onSave(mixer.id, {
			...
			console.error("Failed to save mixer:", error);
		} finally {
			setIsSaving(false);
		}
	};
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-[540px]">
				<DialogHeader>
					<DialogTitle>Edit Mixer: {mixer?.name}</DialogTitle>
				</DialogHeader>
